        ...
        <div class="w-full border border-green-uniceplac pb-4 mt-2 rounded-md">
            <div class="w-full bg-green-uniceplac  grid grid-cols-8 gap-4 p-4">
                ...
                <div class="col-span-3">
                    <select v-on:change="filter()" class="w-full" name="" id="" v-model="currentCareer">
                        <option v-for="career in careers" key="career.id" v-bind:value="career.id">{{career.name}}</option>
                    </select>
                </div>
                <div class="col-start-8 col-span-1 flex justify-end align-center">
                    <a  :href="getRouteExport()"
                        target="_blank"
                        title="Exportar"
                        class="text-white ml-2 text-center capitalize rounded-md bg-green-500 hover:bg-green-600 pl-3 pr-3 py-2 font-bold"
...
export default {
  ...
  methods: {
      ...
      filter: function () {
          this.$inertia.visit(this.route("report.certificates", {career_id: this.currentCareer}));
      },
      getRouteExport: function () {
          return this.route("report.certificates.export", {career_id: this.currentCareer});
      }
  },
  mounted: function (){
